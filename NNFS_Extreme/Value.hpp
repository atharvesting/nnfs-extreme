...
//	*/
//#define DEFINE_MIXED_OPERATOR_PLUS(op_sym, op_char) \
//	void backward_plus(ValuePtr& lhs, ValuePtr& rhs, ValuePtr& out) { \
//		lhs->grad = 1.0f * out->grad; \
//		rhs->grad = 1.0f * out->grad; \
//	} \
//	friend ValuePtr operator op_sym(ValuePtr& lhs, ValuePtr& rhs) { \
//		auto out = std::make_shared<Value>(lhs->data op_sym rhs->data, std::make_pair(lhs, rhs), op_char); \
//		out->_backward = backward_plus(lhs, rhs, out); \
//		return out; \
//	} \
//	friend ValuePtr operator op_sym(ValuePtr& lhs, float& rhs) { \
//		auto r_ptr = std::make_shared<Value>(rhs); \
//		auto out = std::make_shared<Value>(lhs->data op_sym r_ptr->data, std::make_pair(lhs, r_ptr), op_char); \
//		out->_backward = backward_plus(lhs, r_ptr, out); \
//		return out; \
//	} \
//	friend ValuePtr operator op_sym(const float& lhs, const ValuePtr& rhs) { \
//		auto l_ptr = std::make_shared<Value>(lhs); \
//		auto out =  std::make_shared<Value>(l_ptr->data op_sym rhs->data, std::make_pair(l_ptr, rhs), op_char); \
//		out->_backward = backward_plus(l_ptr, rhs, out); \
//		return out; \
//	}
//	DEFINE_MIXED_OPERATOR_PLUS(+, '+')
//	//DEFINE_MIXED_OPERATOR(-, '-')
//	//DEFINE_MIXED_OPERATOR(/ , '/')
//#undef DEFINE_MIXED_OPERATOR_PLUS
//
